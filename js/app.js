...
function migrateTo31(data){
  ...
  migrated.settings={...clone(INITIAL_DATA.settings),...(migrated.settings || {})};
  if(!("jobSearchDate" in migrated.settings)) migrated.settings.jobSearchDate=null;
...
function renderJobSearchCountdown(){
  const iso=state.settings?.jobSearchDate;
  const daysEl=$("#jobSearchDays"),labelEl=$("#jobSearchDaysLabel"),dateEl=$("#jobSearchDateText");
  if(!iso){daysEl.textContent="—";labelEl.textContent="дней";dateEl.textContent="Укажи дату в меню •••";return}
  const today=new Date(`${localDateKey()}T00:00:00`);
  const target=new Date(`${iso}T00:00:00`);
  const days=Math.ceil((target-today)/86400000);
  dateEl.textContent=formatDate(iso);
  if(days<0){daysEl.textContent=Math.abs(days);labelEl.textContent=`${plural(Math.abs(days),"день","дня","дней")} назад`;return}
  if(days===0){daysEl.textContent="Сегодня";labelEl.textContent="";return}
  daysEl.textContent=days;labelEl.textContent=plural(days,"день","дня","дней");
}
function renderDashboard(){
  renderJobSearchCountdown();
...
function renderTopicStatus(topic){
...
function renderTopicMenu(){
  return `<div class="topic-menu-wrap">
    <button class="topic-menu-button" data-action="toggle-topic-menu" aria-label="Действия с темой" aria-expanded="false">⋮</button>
    <div class="topic-menu" hidden>
      <button class="topic-menu-item" data-action="edit-topic">Редактировать</button>
      <button class="topic-menu-item is-danger" data-action="delete-topic">Удалить</button>
    </div>
  </div>`;
}
function renderLearningDesktopSection(section){
  return `<div class="learning-topics-desktop">
          ...
          return `<tr data-topic-id="${escapeHtml(topic.id)}">
            <td>
              <div class="topic-title">
                <div class="topic-leading">
                  ${renderTopicMenu()}
                  <div class="topic-label-wrap">
                    ${renderTopicStatus(topic)}
                    <span class="${completed && !topic.review ? "topic-complete" : ""}">${escapeHtml(topic.title)}</span>
                  </div>
                </div>
              </div>
            </td>
...
function renderLearningMobile(section){
  const cards=section.topics.map(topic=>{
    const completed=topic.theory&&topic.practice;
    return `<div class="topic-card" data-topic-id="${escapeHtml(topic.id)}">
      <div class="topic-card-header">
        <div class="topic-card-leading">
          ${renderTopicMenu()}
          ${renderTopicStatus(topic)}
        </div>
        <span class="topic-card-title ${completed && !topic.review ? "topic-complete" : ""}">${escapeHtml(topic.title)}</span>
      </div>
      <div class="topic-card-body">
        <div class="topic-card-row">
          <span class="topic-card-label">Теория</span>
          <input class="check" type="checkbox" data-action="toggle-theory" ${topic.theory?"checked":""}>
        </div>
        <div class="topic-card-row">
          <span class="topic-card-label">Практика</span>
          <input class="check" type="checkbox" data-action="toggle-practice" ${topic.practice?"checked":""}>
        </div>
      </div>
    </div>`;
  }).join("")||`<div class="topic-card topic-card-empty"><p class="muted">Тем пока нет</p></div>`;
  return `<div class="topic-card-list learning-topics-mobile">${cards}</div>`;
}
function renderLearning(){
  ...
  <button class="text-button section-add-topic" data-action="add-topic-to-section">+ Тема</button>
  <button class="text-button" data-action="edit-section">Изменить</button>

  <div class="section-order-actions" aria-label="Изменить порядок раздела">
    <button
      class="section-order-button"
      data-action="move-section-up"
      aria-label="Переместить раздел вверх"
      ${state.learning[0]?.id===section.id?"disabled":""}
    >↑</button>
    ...
    <button
      ...
      ${state.learning.at(-1)?.id===section.id?"disabled":""}
    >↓</button>
  </div>
</div>
...
<button class="text-button danger-text" data-action="delete-section">Удалить</button>
      ...
      ${renderLearningDesktopSection(section)}
      ${renderLearningMobile(section)}
    </article>`
  }).join("")
}
...
function closeTopicMenus(exceptMenu=null){
  $$(".topic-menu").forEach(menu=>{
    if(menu===exceptMenu) return;
    menu.hidden=true;
    const button=menu.closest(".topic-menu-wrap")?.querySelector(".topic-menu-button");
...
document.addEventListener("click",event=>{
  ...
  if(event.target.closest("#jobSearchDateButton")){
    $("#jobSearchDateInput").value=state.settings?.jobSearchDate || "";
    openSettings(false);
    return $("#jobSearchDateDialog").showModal();
  }
  ...
  if(action==="toggle-topic-menu"){
    event.stopPropagation();
    const wrap=event.target.closest(".topic-menu-wrap");
    const menu=wrap?.querySelector(".topic-menu");
    if(!menu) return;
    const willOpen=menu.hidden;
    closeTopicMenus(menu);
  ...
  if(action==="add-topic-to-section"){
    openDialog("add-topic");
    $("#dialogSectionSelect").value=section.id;
  }
  if(action==="move-section-up" || action==="move-section-down"){
    const index=state.learning.findIndex(item=>item.id===section.id);
    const nextIndex=action==="move-section-up"?index-1:index+1;
    if(nextIndex>=0 && nextIndex<state.learning.length){
      [state.learning[index],state.learning[nextIndex]]=[state.learning[nextIndex],state.learning[index]];
      saveState({activityType:"edit"});
    }
  }
  ...
  if(action==="edit-topic"){closeTopicMenus();openDialog("edit-topic",{section,topic});}
  if(action==="delete-topic"){closeTopicMenus();if(confirm(`Удалить тему «${topic.title}»?`)){section.topics=section.topics.filter(t=>t.id!==topic.id);saveState({activityType:"edit"})}}
...
$("#jobSearchDateForm").addEventListener("submit",event=>{
  event.preventDefault();
  state.settings.jobSearchDate=$("#jobSearchDateInput").value || null;
  $("#jobSearchDateDialog").close();
  saveState();
});
$("#closeJobSearchDateDialog").addEventListener("click",()=>$("#jobSearchDateDialog").close());
$("#cancelJobSearchDateButton").addEventListener("click",()=>$("#jobSearchDateDialog").close());
renderAll();
...
document.addEventListener("click", event=>{
  if(!event.target.closest(".topic-menu-wrap")) closeTopicMenus();
});
